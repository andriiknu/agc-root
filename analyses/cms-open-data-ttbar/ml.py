...
def define_cpp (fastforest_path, MAX_N_JETS = 6):
    import os
...
def prepare (df: ROOT.RDataFrame)->ROOT.RDataFrame:
...
def define_features (df: ROOT.RDataFrame)->ROOT.RDataFrame:
    ...
    df2 = (
        df2.Define(
            'W1_idx', 
            f'permutations[std::min(Jet_pt_cut.size(), MAX_N_JETS)][0]'
        )
        .Define(
            'W2_idx', 
            f'permutations[std::min(Jet_pt_cut.size(), MAX_N_JETS)][1]'
        )
        .Define(
            'bH_idx',
            f'permutations[std::min(Jet_pt_cut.size(), MAX_N_JETS)][2]'
        )
        .Define(
            'bL_idx',
            f'permutations[std::min(Jet_pt_cut.size(), MAX_N_JETS)][3]'
        )
    )
    ...
    #         # 12 features
            ...
            .Define(f'Jet_qgl_cut', 'Jet_qgl[Jet_mask]')
    ...
    df2 = (
        ...
        df2.Define(
            ...
            f'{feature_names[4]}', 'return Map(JetbL_p4+Lepton_p4.at(0), [] (const ROOT::Math::PxPyPzMVector &p) {return p.M();})'
        ).Define(
            f'{feature_names[5]}', 'return Map(JetW1_p4+JetW2_p4, [] (const ROOT::Math::PxPyPzMVector &p) {return p.M();})'
        ).Define(
            f'{feature_names[6]}', 'return Map(JetW1_p4+JetW2_p4+JetbH_p4, [] (const ROOT::Math::PxPyPzMVector &p) {return p.M();})'
        ).Define(
            f'{feature_names[7]}', 'return Map(JetW1_p4+JetW2_p4+JetbH_p4, [] (const ROOT::Math::PxPyPzMVector &p) {return p.Pt();})'
        )
    ) 
    # put all features into one collection
    feature_names = feature_names.__str__().replace("[","{").replace("]","}").replace("'","")
    df2 = df2.Define("features",  
        f"ROOT::VecOps::RVec<ROOT::RVecF>({feature_names})"
        )
...
def infer (df: ROOT.RDataFrame)->ROOT.RDataFrame:

    df2 = predict(df)
    for i, feature in enumerate(features["names"]):
        df2 = df2.Define(f"results{i}", f"{feature}[perm_idx]")
